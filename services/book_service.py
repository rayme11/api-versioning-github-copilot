...
class BookService:
    ...
    async def get_book_by_id(self, book_id: str) -> Book:
        ...
        book = await self.book_repository.get_one_book(book_id)
        if not book:
    ...
    async def soft_delete_book(self, book_id: str) -> bool:
        if not ObjectId.is_valid(book_id):
            raise InvalidBookDataError("Invalid book ID format.")
        try:
            success = await self.book_repository.soft_delete_book(book_id)
            if not success:
    ...
    async def update_book(self, book_id: str, book: Book) -> bool:
        ...
        try:
            ...
            raise BookServiceError(f"Failed to update book: {e}")
